...
export default ({ config, db }) => resource({
	...
	index({ query, facet }, res) {
		...
		// const Skip = (skip) ? skip : 1;
		// const Limit = (limit && limit <= 100) ? limit : 50;

		// const currentPage = (skip) ? skip : 1;
		// const perPage = (limit && limit <= 100) ? limit : 50;
		// const Skip = (perPage * ((skip) ? skip : 1)) - perPage;
		...
		// 			{
		// 				$addFields : {
		// 					hasNextPage: { $cond: [{ $lt: [ '$currentPage', {$ceil:{ $divide: [ "$total", '$perPage' ] }} ] }, true, false ] },
		// 				}
		// 			}

		// 		],
		// 		data: [
		// 			{ $match: filter || {document: "$$ROOT"} },
		// 			{ $project: projection || { _id: 1} },
		// 			{ $skip: Skip || 0 },
		// 			{ $limit: perPage || 1 },
		// 			// { $sort: sort || {document: "$$ROOT"} },
		// 		]
		// 	} }
		// ]).exec((err, result) => {
		// 	res.json(result);
